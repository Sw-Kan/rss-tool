...
- 列表、阅读器、设置弹窗仍是「文章 = 左列表 + 右正文」两栏。图片/视频没有右栏：详情走一个 960×760 的弹层（`MediaDetailOverlay`），因为把正文挤成右栏会把 6 列瀑布流压到 2–3 列。弹层里没有阅读进度条与 AI 按钮（两者都只属于 `article`）。
- 侧边栏源行可以**鼠标长按拖拽**到别的目录（或拖回「未分组源」）：长按约 250ms 后拖动，拖影跟着指针、落点行高亮，松手即改所属目录；Esc 取消。**触摸端不做**（与侧边栏滚动打架），改目录仍可去「设置 → 订阅源 → 编辑」。落点只有目录行与「未分组源」区块，不支持拖到目录里已展开的源上或拖动排序。
...
- 抓取是**直连优先**：每一跳先直连（连接超时上限 `_DIRECT_CONNECT_TIMEOUT` = 5s，读写超时照旧），只有连接层失败（`ConnectError` / `ConnectTimeout` / `ProxyError`，即连接被拒、RST、TLS 失败、代理端口没人听）才回退代理；读写超时（连上了只是慢）不换路，直接报「请求超时」。所以代理进程（Clash 之类）没起来时国内源照常刷新，需要代理的源仍能走代理。代价：需要代理的源每次刷新先白试一次直连，被 DNS 污染/黑洞的源最多多等 5s。
- 抓取在 `custom` 与 `system` 两种模式下都是直连优先：`system` 的第二条路是环境变量里的代理（靠 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 粗略探测，不精确解析 httpx 的 env 优先级），环境里没有代理变量就只直连。**这会让「所有流量必须走企业代理」的部署默认落到直连**。`NO_PROXY` 命中（域名 / 后缀 / CIDR）则只直连，不回退代理。
- 单实例、无迁移、无密码找回、无登录限流。
