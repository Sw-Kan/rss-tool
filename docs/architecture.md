...
集成（integrations，按 kind 一行，config 是 JSON）
  rsshub        base_url + access_key（只管「怎么连」）
  obsidian      vault_path
  feishu        webhook_url
  custom_export endpoint

  顶点用法一：POST /api/feeds 收到裸路由（/sspai/matrix）→ expand_route() 用 base_url 拼成
              完整地址，有 ACCESS_KEY 就补 ?key=
  顶点用法二：被自动化的动作消费（推送飞书 / 写入 Obsidian / 推自定义接口）

  RSSHub 自己的配置（cookie / refresh_token / cache）在 RSSHub 侧维护：rss-tool 既不给它
  注入也不读取（早期版本试过把参数拼进 URL / 生成 env 片段，两条都不通：前者把凭据写进
  共享的 feeds.url，后者仍然得用户重启容器才生效），所以集成页只留地址 / 密钥 / 测试连接。
