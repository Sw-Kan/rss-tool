...
- [ ] 抓取直连优先：代理填一个**没在跑**的端口 → 刷新国内源仍成功（走直连），被墙源报「直连与代理都不通」；`NO_PROXY` 填该源域名 → 报「NO_PROXY 命中，未尝试代理」
...
- [ ] 侧边栏里在源行上长按约 0.25 秒再拖到某个目录上松手 → 源换目录、计数跟着变、刷新后仍然在那（短点一下是切源，不应移动）
- [ ] 展开某个目录，把它里面的源拖到「未分组源」区块松手 → 源回到未分组（`sqlite3 … "select folder_id from subscriptions"` 验为 NULL）；拖动中按 Esc 应取消
- [ ] 图片页点任意一张卡片 → 弹层打开（大图 + 标题 + 来源）；× / Esc / 点遮罩都能关，且关掉后按浏览器后退**不会**又弹开（历史里不留选中态）
- [ ] 视频页点任意一张卡片 → 弹层里是 16:9 封面 + 居中播放按钮，点它在新标签打开原站播放；深链 `?kind=video&item=…` 刷新后弹层直接出现，弹层里按「下一条」内容跟着换、`item=` 同步更新
